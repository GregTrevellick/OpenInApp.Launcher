...
Open multiple files simultaneously in [OpenInVS2015][ThirdPartyAppHomePage] directly from [Visual Studio's][VisualStudioURL] Solution Explorer.
...
- Open multiple files simultaneously in [OpenInVS2015][ThirdPartyAppHomePage] with VS2012, VS2013 and VS2015.
...
- Warning if attempting to open one or more file types whose extension is not typically associated with [OpenInVS2015][ThirdPartyAppHomePage]. The list of typical file extensions is configurable in Tools > Options.

  ![](../ReadMeScreenShot_WarningNonTypical.png)

- Option to supress the warning message if attempting to open a file whose extension is not typically associated with [OpenInVS2015][ThirdPartyAppHomePage].
...
The [owner](https://github.com/GregTrevellick) of this [GitHub repository / software][GitHubRepoURL] is not affiliated, associated, authorized, endorsed by, employed by, sponsored by, or in any way officially connected with [OpenInVS2015][ThirdPartyAppHomePage] or any of its subsidiaries or its affiliates.

Nor has [this][GitHubRepoURL] software been authorised, approved, verified or in anyway assessed by [OpenInVS2015][ThirdPartyAppHomePage], or any of its subsidiaries or its affiliates, either as [raw source code][GitHubRepoURL] on [GitHub.com](https://github.com/) or as a [Visual Studio Extension][VSMarketplaceUrl] in the [Visual Studio Marketplace](https://marketplace.visualstudio.com/vs).

All Trademark, intellectual property rights, and other rights belonging to [OpenInVS2015][ThirdPartyAppHomePage] as described in [here][ThirdPartyAppHomePage] apply.

All [OpenInVS2015][ThirdPartyAppHomePage] logos and [OpenInVS2015][ThirdPartyAppHomePage] links belong to [OpenInVS2015][ThirdPartyAppHomePage] and their use here and any associated goodwill inures to [OpenInVS2015][ThirdPartyAppHomePage].

In no event shall [OpenInVS2015][ThirdPartyAppHomePage] be liable for any claim, damages or other liability, whether in an action of contract, tort or otherwise, arising from, out of or in connection with the software or the use or dealings in the software.
...
Thanks also to [OpenInVS2015][ThirdPartyAppHomePage] themselves.
